...
public class GamefieldController implements Initializable {
    ...
    public static Node getPlayerNode(StackPane sp, int index) {
        ...
        // there is only the GridPane before the players in the StackPane, so offset by
        // one
        return sp.getChildren().get(index + 1);
    }

    // the height of a rectangle may be equal to the witdth of the field and
    // viceversa, due to rotation
    private final double defaultFieldHeight = 50.0;
    private final double defaultFieldWidth = 100.0;

    // height and width of cornerfield are equal to the value of defaultFieldWidth
    // if you change the defaultFieldWidth you should also resize the
    // cornerFieldLength
    private final double cornerFieldLength = 100.0;
    ...
    private void setupRow(int x, int y, int factor, boolean subtract, boolean horizontal) {
        for (int i = 1; i <= 9; i++) {
            ...
            // checks whether or not a field is a streetField and sets isStreetField
            // accordingly
            ...
            // draws the base fields based on it's properties (StreetField, not StreetField
            // etc.)
            Rectangle rec = new Rectangle();
            rec.setHeight(horizontal ? (isStreetField ? defaultFieldWidth * 3 / 4 : defaultFieldWidth)
                    : (defaultFieldHeight));
            rec.setWidth(horizontal ? (defaultFieldHeight)
                    : (isStreetField ? defaultFieldWidth * 3 / 4 : defaultFieldWidth));
            ...
            // StreetFields also receive there specific color, that is shown an upper
            // rectangle + name and price are added to label
            if (isStreetField) {
            ...
            // adds the base rectangle & topper one (for the streets) according to it's
            // orientation
            ...
            if (!horizontal) {
                ...
                if (factor == 20) {
                    ...
                    cardStack.getChildren().addAll(box, name, price);
                    StackPane.setAlignment(name, Pos.CENTER);
                    StackPane.setAlignment(price, Pos.CENTER_RIGHT);
                }
            } else {
                box = new VBox();
                if (factor == 20) {
                    ...
                    cardStack.getChildren().addAll(box, name, price);
                    StackPane.setAlignment(name, Pos.CENTER);
                    StackPane.setAlignment(price, Pos.TOP_CENTER);
    ...
    private void showStreetCard(StreetField field) {
        ...
        StackPane.setAlignment(streetName, Pos.CENTER);
        StackPane.setAlignment(streetPrice, Pos.BOTTOM_CENTER);
