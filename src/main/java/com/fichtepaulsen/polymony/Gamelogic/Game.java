...
public class Game implements GameInterface {
    ...
    public void startGame(int playerCount) {
        ...
        try {
            fields = readJson();
                ...
                | NoSuchMethodException | InvocationTargetException | JSONException e) {
            Logger.getLogger(Game.class.getName()).log(Level.SEVERE, e.getMessage());
        }
    ...
    public int[] rollDices() {
        ...
        if (activePlayer.getIsInPrison() == true) {
            ...
            if (doublets == false) {
                ...
                if (activePlayer.getDoubletsCounter() == 3) { // When doubletCounter reaches 3, the player will be
    ...
    public boolean isAbleToUseGetOutOfJailCard() {
        Player activePlayer = players[activePlayerIndex];
        return activePlayer.getIsInPrison() == true && activePlayer.getAmountPrisonFreeCard() >= 1;
    }
    ...
    public Field[] readJson() throws IOException, JSONException, NoSuchMethodException, ClassNotFoundException,
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            ...
            try {
                // Je nachdem welche Klasse es ist wird der Konstruktor mit den jeweils
                // gewünschten Werten aufgerufen und das Objekt in temp an Stelle des Indizes i
                // geschrieben
                switch (fieldClassName) {
                    ...
                    case "StreetField":
                        temp[i] = new StreetField((String) field.get("name"), (int) field.get("price"),
                                getColor((int) field.get("color")), (int) field.get("rent"), (int) field.get("house1"),
                                (int) field.get("house2"), (int) field.get("house3"), (int) field.get("house4"),
                                (int) field.get("hotel"));
                        break;
                    case "ActionField":
                        try {
                    ...
                    case "TaxField":
                        ...
                        break;
                    case "TrafficField":
                        temp[i] = new TrafficField((String) field.get("name"), (int) field.get("price"),
                                (int) field.get("rent"));
                        break;
                    case "Prison":
    ...
    public void readCardsJson() throws IOException {
        ...
        Card[] localChanceCards = new Card[chanceCardsJson.length()];
        ...
        for (int i = 0; i < chanceCardsJson.length(); i++) {
            ...
            switch (cardClassName) {
                case "MoneyCard":
                    localChanceCards[i] = new MoneyCard((String) card.getString("text"), (int) card.get("value"),
                            (boolean) card.get("community"));
                    break;
                case "JumpCard":
                    localChanceCards[i] = new JumpCard(card.getInt("value"), card.getString("text"),
                            card.getBoolean("community"));
                    break;
                case "JumpToCard":
                    localChanceCards[i] = new JumpToCard(card.getInt("position"), card.getString("text"),
                            card.getBoolean("money"), card.getBoolean("community"));
                    break;
                case "MoneyCardOtherPlayers":
                    localChanceCards[i] = new MoneyCardOtherPlayers((String) card.getString("text"),
                            (int) card.get("value"), (boolean) card.get("community"));
                    break;
                case "PrisonFreeCard":
                    localChanceCards[i] = new PrisonFreeCard((String) card.getString("text"),
                            (boolean) card.get("community"));
                    break;
                case "GoToPrisonCard":
                    localChanceCards[i] = new GoToPrisonCard((String) card.getString("text"),
                            (boolean) card.get("community"));
                    break;
                default:
        ...
        this.chanceCards = localChanceCards;

        JSONArray communityCardsJson = cardsObj.getJSONArray("community");

        Card[] localCommunityCards = new Card[communityCardsJson.length()];
        ...
        for (int i = 0; i < communityCardsJson.length(); i++) {
            ...
            switch (cardClassName) {
                case "MoneyCard":
                    localCommunityCards[i] = new MoneyCard((String) card.getString("text"), (int) card.get("value"),
                            (boolean) card.get("community"));
                    break;
                case "JumpCard":
                    localCommunityCards[i] = new JumpCard(card.getInt("value"), card.getString("text"),
                            card.getBoolean("community"));
                    break;
                case "JumpToCard":
                    localCommunityCards[i] = new JumpToCard(card.getInt("position"), card.getString("text"),
                            card.getBoolean("money"), card.getBoolean("community"));
                    break;
                case "MoneyCardOtherPlayers":
                    localCommunityCards[i] = new MoneyCardOtherPlayers((String) card.getString("text"),
                            (int) card.get("value"), (boolean) card.get("community"));
                    break;
                case "PrisonFreeCard":
                    localCommunityCards[i] = new PrisonFreeCard((String) card.getString("text"),
                            (boolean) card.get("community"));
                    break;
                case "GoToPrisonCard":
                    localCommunityCards[i] = new GoToPrisonCard((String) card.getString("text"),
                            (boolean) card.get("community"));
                    break;
                default:
                    Logger.getLogger(getClass().getName()).log(Level.SEVERE, "Card JSON import not working!");
        ...
        this.communityCards = localCommunityCards;
    }
    ...
    public boolean isAbleToBuyField() {
        ...
        if (currentField instanceof OwnableField) {
            ...
            return activePlayer.getBalance() >= oField.getPrice() && oField.getOwner() == null;
        } else {
            return false;
        }
    }
    ...
    /*
     * requires:
     * does: current player pays the rent on the ownableField he stands on to the
     * field owner
     */
    public void payRent() {
        Player activePlayer = getCurrentPlayer();
    ...
    /*
     * requires:
     * returns: boolean if the current player has to pay rent on the field he stands
     * on
     */
    public boolean hasToPayRent(Player activePlayer, Field currentField) {
        ...
        if (currentField instanceof OwnableField) {
            OwnableField oField = (OwnableField) fields[activePlayer.getPosition()];
            return oField.getOwner() != activePlayer && oField.getOwner() != null && !oField.getIsMortgage();
        } else {
            return false;
        }
    }
    ...
    public boolean pastStart(int lastPosition, int newPosition) {
        return (newPosition - lastPosition) < 0;
    }
     ...
     * returns: boolean if the current player is able to add a mortgage at a field
     * at fieldIndex
     */
    public boolean isAbleToAddMortgage(int fieldIndex) {
        Player activePlayer = players[activePlayerIndex];
        OwnableField oField = (OwnableField) fields[fieldIndex];
        return activePlayer == oField.getOwner() && !oField.getIsMortgage();
    }
    ...
    /*
     * requires: index of the field where a player wants to remove the mortgage
     * returns: boolean if the current player is able to remove a mortgage from a
     * field at fieldIndex
     */
    public boolean isAbleToRemoveMortgage(int fieldIndex) {
        Player activePlayer = players[activePlayerIndex];
        OwnableField oField = (OwnableField) fields[fieldIndex];
        final boolean isOwner = activePlayer == oField.getOwner();
        final boolean canAffordMortgage = activePlayer
                .getBalance() >= (oField.getMortgageAmount() + oField.getMortgageAmount() * 1 / 10);

        return isOwner && oField.getIsMortgage() && canAffordMortgage;
    }
    ...
    @Override
    // effect: buys a house or hotel on the given field, follows automaticly the
    // rules
    public void buyHouse(int fieldIndex) {
        StreetField currentField = (StreetField) fields[fieldIndex];
        if (currentField.getHouseamount() < 4) { // checks if the field is a StreetField and if there´s space on the
                                                 // field
